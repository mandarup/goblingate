...
def relay_logic_upper(relay, upper_tank, lower_tank, clock_times):
    action_time = False
    ...
    if relay.start_time is not None and relay.stop_time is None:
        print(dt.datetime.now(), relay.start_time)
        dur = dt.datetime.now() - relay.start_time
...
def relay_logic_lower(relay, lower_tank, clock_times):
    action_time = False
    now = dt.datetime.now().time()
    for t in clock_times:
        if t.start <= now < t.stop:
    ...
    if action_time:
        if not lower_tank.is_high:
            if not relay.is_on:
                print('setting lower relay ON {}'.format(relay.is_on))
                relay.update(SIGNAL.ON)
            else:
    ...
    print('lower relay state: {}'.format(relay.is_on))
...
def test_init():
    ...
    lower_tank_relay = RelayState(duration=20)
        ...
        Period(
            ...
            (now +
             relativedelta.relativedelta(
                 seconds=300)).time()),
        Period((now + relativedelta.relativedelta(seconds=620)).time(),
    ...
    clock_times_lower = [
        Period(
            ...
            (now +
             relativedelta.relativedelta(
                 seconds=600)).time()),
        Period((now + relativedelta.relativedelta(seconds=700)).time(),
